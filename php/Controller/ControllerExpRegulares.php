...
class ControllerExpRegulares extends Controller {
    ...
    public function geradorTabelaAutomatoFinito($sDados) {
        ...
        //Estado 0
        $this->Model->aTabelaAutomato[$this->Model->iPos][] = $this->Model->iPos;
        $this->Model->aTabelaAutomato[$this->Model->iPos][] = '?';
        ...
        while (count($this->Model->aArrayEstTokenExpr) >= $this->Model->iPos) {
            $sVal = $this->Model->aArrayEstTokenExpr[$this->Model->iPos]; //Token, expressão
            $this->Model->aTabelaAutomato[$this->Model->iPos][] = trim($sVal[0]); //Seta o estado de cada expressão
            ...
            foreach ($this->Model->aArrayCaracteres as $sChar) {
                ...
                if ($sVal[0] == "?") { //Se for ? é por que é um token composto, estado de transição e não de aceitação
                    ...
                    if (strlen($sVal[1]) == 2) {
                        if ($this->oModel->aArray1[1] == $sChar) {
                            $this->oModel->iEst++;
                            $this->Model->aArrayEstTokenExpr[$this->oModel->iEst] = [$sVal[2], $this->oModel->aArray1[1]];
                            $this->Model->aTabelaAutomato[$this->Model->iPos][] = $this->oModel->iEst;
                            $bCont = false;
                        }
                    }
                    ...
                    if (count($this->oModel->aArray1) > 2) {
                        if ($this->oModel->aArray1[1] == $sChar) {
                            $this->oModel->iEst++;
                            $this->Model->aArrayEstTokenExpr[$this->oModel->iEst] = ["?", substr($sVal[1], 1), $sVal[2]];
                            $this->Model->aTabelaAutomato[$this->Model->iPos][] = $this->oModel->iEst;
                            $bCont = false;
                        }
                    }
                    ...
                    foreach ($sArrayTokenExpr2 as $key => $sExprr) {
                        ...
                        if ((preg_match("/" . $sVal[1] . "/", $this->oModel->aArray1[1]) == 1) && $this->oModel->aArray1[1] == $sChar) {
                            ...
                            if (strlen(substr($sExprr, 1)) > 2) {
                                $this->Model->aArrayEstTokenExpr[$this->oModel->iEst] = ['?', substr($sExprr, 1), $key, $this->Model->iPos, $sVal[1]]; //$sVal[0]
                            } else {
                                $this->Model->aArrayEstTokenExpr[$this->oModel->iEst] = [$key, substr($sExprr, 1), $key, $this->Model->iPos, $sVal[1]];
                            }
                            $this->Model->aTabelaAutomato[$this->Model->iPos][] = $this->oModel->iEst;
                ...
                if (isset($sVal[3]) && $sVal[0] != "?") {
                    if (preg_match("/" . $sVal[4] . "/", $sChar) == 1 && $sChar != "\\t" && $sChar != "\\n" && $sChar != "\\r") {
                        if ($sVal[0] == $sVal[2]) {
                            //    $this->Model->aTabelaAutomato[$this->Model->iPos][] = $sVal[3]; 
                            //    $bCont = false;
                        } else {
                            $this->oModel->aArray1 = str_split($sVal[1]);
                            if ((preg_match("/" . $sVal[4] . "/", $this->oModel->aArray1[1]) == 1) && $this->oModel->aArray1[1] == $sChar) {
                                ...
                                if (strlen(substr($sVal[1], 1)) > 1) {
                                    $this->Model->aArrayEstTokenExpr[$this->oModel->iEst] = ['?', substr($sVal[1], 1), $sVal[2], $sVal[3], $sVal[4]]; //$sVal[0]
                                } else {
                                    $this->Model->aArrayEstTokenExpr[$this->oModel->iEst] = [$sVal[2], substr($sVal[2], 1), $sVal[2], $sVal[3], $sVal[4]];
                                }
                                $this->Model->aTabelaAutomato[$this->Model->iPos][] = $this->oModel->iEst;
                                $bCont = false;
                            } else {
                ...
                if (count($this->Model->aArrayTokenExpr) > 0) {
                    foreach ($this->Model->aArrayTokenExpr as $key => $sExprr) {
                        ...
                        foreach ($aArrayComp as $sKey1 => $sLexic) {
                            if (trim($sVal[0]) == trim($sLexic) && isset($aArrayComp[$sKey1 + 1])) {
                                ...
                                if ((preg_match("/" . $sExp2 . "/", $sChar) == 1) && $sChar != "\\t" && $sChar != "\\n" && $sChar != "\\r") {
                                    ...
                                    $this->Model->aTabelaAutomato[$this->Model->iPos][] = $this->oModel->iEst; //paralelo
                                    $this->Model->aArrayEstTokenExpr[$this->oModel->iEst] = [$key, $aArrayComp];
                                    $bCont = false;
                                }
                            }
                        }
                ...
                if ($bCont) {
                    $this->Model->aTabelaAutomato[$this->Model->iPos][] = -1;
                }
            }
            $this->Model->iPos++;
            if (count($this->Model->aArrayEstTokenExpr) >= $this->Model->iPos) {
                $this->Model->aTabelaAutomato[$this->Model->iPos][] = $this->Model->iPos;
            }
        }
    ...
    public function retornaArrayTokenExp() {
        foreach ($this->oModel->aArray as $sVal1) {
            ...
            if (strpos($sVal1, '\:') !== false) {
                ...
                $this->Model->aArrayTokenExpr[trim($aArray2[0])] = ":";
            } else {
                $aArray2 = explode(':', $sVal1);
                $this->Model->aArrayTokenExpr[trim($aArray2[0])] = trim($aArray2[1]);
            }
        }
    }
    ...
    public function preencheDadosInicial($bCont) {
        if ($this->Model->sExp != $this->oModel->aArray1[1]) {
            $this->oModel->iEst++;
            $this->Model->aArrayEstTokenExpr[$this->oModel->iEst] = $this->oModel->aArray1;
            //Parte que retira as expressões que possuem estado (Ficar só compostas)
            if (isset($this->Model->aArrayTokenExpr[$this->oModel->aArray1[0]])) {
                unset($this->Model->aArrayTokenExpr[$this->oModel->aArray1[0]]);
            }
        }
        $this->Model->aTabelaAutomato[$this->Model->iPos][] = $this->oModel->iEst;
        $bCont = false;
        $this->Model->sExp = $this->oModel->aArray1[1];
    ...
    public function montaEstadoInicial() {
        ...
        foreach ($this->Model->aArrayCaracteres as $sChar) {
                    ...
                    if (!(trim($this->oModel->aArray1[0]) == trim($this->oModel->aArray1[1]))) {
                        ...
                        if ($sChar != "\\t" && $sChar != "\\n" && $sChar != "\\r" && $sChar != "{" || $sEscapaCol) {
                            ...
                            if ($bCont && substr_count($this->oModel->aArray1[1], $sChar) == strlen($this->oModel->aArray1[1]) && strlen($this->oModel->aArray1[1]) > 1) {
                                if ($this->Model->sExp != $this->oModel->aArray1[1]) {
                                    $this->oModel->iEst++;
                                    $this->Model->aArrayEstTokenExpr[$this->oModel->iEst] = ["?", $this->oModel->aArray1[1], $this->oModel->aArray1[0]]; //Adiciona o token
                                    //Parte que retira as expressões que possuem estado (Ficar só compostas)
                                    if (isset($this->Model->aArrayTokenExpr[$this->oModel->aArray1[0]])) {
                                        unset($this->Model->aArrayTokenExpr[$this->oModel->aArray1[0]]);
                                    }
            ...
            //Coloca -1 em todas as posições que não possuem transição na tabela
            if ($bCont) {
                $this->Model->aTabelaAutomato[$this->Model->iPos][] = -1; //paralelo
            }
        }
    }
